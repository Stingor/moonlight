...
du systeme renewal vers des valeurs compatibles pre-renewal.
...
Formule DEF/MDEF :
  ...
  - Resultat plafonne entre 1 et DEF_CAP / MDEF_CAP

Formule EXP :
  new_base_exp = round(BaseExp * EXP_BASE_MULT)
  new_job_exp  = round(JobExp  * EXP_JOB_MULT)
  (multiplicateurs configurables, par defaut 0.70 / 0.50)

Usage :
  python re_to_prere_mob.py <fichier.yml>            # modifie le fichier en place
  python re_to_prere_mob.py <fichier.yml> -o <out>   # Ecrit dans un nouveau fichier
...
EXP_BASE_PER_HP = 1.6    # base exp = HP * ce ratio (ex: 200k HP -> 400k base exp)
...
def split_mob_blocks(content):
    """
    Decoupe le fichier en deux parties : l'en-tete (avant Body:)
...
def parse_int_field(block, field):
    """Extrait la valeur entiere d'un champ YAML simple (ex: Level: 145 ou - Id: 145)."""
    m = re.search(r'^\s+(?:- )?' + field + r':\s*(\d+)', block, re.MULTILINE)
    return int(m.group(1)) if m else None


def has_flag(block, flag):
    """Verifie si un flag boolen est present et a true."""
...
def convert_file(input_path, output_path=None, dry_run=False, convert_def=True, convert_exp=True, convert_hp=True):
    ...
    if not blocks:
        print("Aucun bloc 'Body:' trouve dans le fichier.")
        return
...
def main():
    parser = argparse.ArgumentParser(
        description='Convertit Defense/MagicDefense renewal en valeurs compatibles pre-renewal dans un mob_db.yml'
    )
    parser.add_argument('input', help='Fichier YML a convertir')
    parser.add_argument('-o', '--output', default=None,
                        help='Fichier de sortie (par defaut : modifie en place)')
