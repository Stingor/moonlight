...
bool mapif_parse_itembound_retrieve(int32 fd)
{
	...
	for( j = 0; j < MAX_ITEM_RDM_OPT; ++j ) {
		stmt.BindColumn(12+MAX_SLOTS+j*3, SQLDT_SHORT, &item.option[j].id, 0, nullptr, nullptr);
		stmt.BindColumn(13+MAX_SLOTS+j*3, SQLDT_SHORT, &item.option[j].value, 0, nullptr, nullptr);
