...
int unit_warp(struct block_list *bl,short m,short x,short y,clr_type type)
{
	...
	switch (bl->type) {
		case BL_MOB:
			...
			if (m != bl->m && !map_getmapflag(m, MF_NOBRANCH) && battle_config.mob_warp&4 && !(((TBL_MOB *)bl)->master_id)) // inversion MF_NOBRANCH [Stingor]
				return 1;
			break;
		case BL_PC:
...
int unit_skilluse_id2(struct block_list *src, int target_id, uint16 skill_id, uint16 skill_lv, int casttime, int castcancel)
{
	...
	if( status_get_class(target) == MOBID_EMPERIUM ) { // [Stingor]
		switch(skill_id) { // skill autorisez sur l'emp
			case AL_HEAL:
			case PR_SANCTUARY:
				break;
			default:
