...
#ifdef NEW_CARTS
	sd->status.option = sd->sc.option&(OPTION_INVISIBLE|OPTION_FALCON|OPTION_RIDING|OPTION_DRAGON|OPTION_WUG|OPTION_WUGRIDER|OPTION_MADOGEAR|OPTION_WINGS);
#else
	sd->status.option = sd->sc.option&(OPTION_INVISIBLE|OPTION_CART|OPTION_FALCON|OPTION_RIDING|OPTION_DRAGON|OPTION_WUG|OPTION_WUGRIDER|OPTION_MADOGEAR|OPTION_WINGS);
#endif
...
bool pc_authok(struct map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	// [Stingor] -->
	sd->hom_teleport_tick = tick;
	sd->aoe_teleport_tick = tick;
	...
	ShowInfo("'" CL_WHITE "%s" CL_RESET "' logged in."
	         " (AID/CID: '" CL_WHITE "%d/%d/%d" CL_RESET "',"
	         " IP: '" CL_WHITE "%d.%d.%d.%d" CL_RESET "',"
	         " Group '" CL_WHITE "%d" CL_RESET "').\n",
	         sd->status.name, sd->status.account_id, sd->status.char_id, sd->status.user_id,
...
void pc_reg_received(struct map_session_data *sd)
{
	...
	sd->state.active = 1;
	// [Stingor] -->
	sd->state.autoloot = pc_readglobalreg(sd, add_str("autoloot"));
	...
	sd->state.autolootpognon = pc_readglobalreg(sd, add_str("alootpognon"));
...
void pc_bonus(struct map_session_data *sd,int type,int val)
{
	...
	switch(type){
		...
		case SP_RESTART_FULL_RECOVER:
			if(sd->state.lr_flag != 2 && !map_flag_vs(sd->bl.m)) // Kaizel [Stingor]
		...
		case SP_NO_BOTTLE:
			if(sd->state.lr_flag != 2)
				sd->special_state.no_bottle = 1;
			break;
		case SP_NO_AMMO:
			if(sd->state.lr_flag != 2)
				sd->special_state.no_ammo = 1;
			break;
		case SP_NO_ZENY:
			if(sd->state.lr_flag != 2)
...
bool pc_skill(struct map_session_data* sd, uint16 skill_id, int level, enum e_addskill_type type) {
	...
	if (!skill_id || !(idx = skill_get_index(skill_id))) {
		ShowError("pc_skill: Skill with id %d does not exist in the skill database (%s)\n", skill_id, sd->status.name);
		return false;
	}
 ...
 */
int pc_paycash(struct map_session_data *sd, int price, int points, e_log_pick_type type, int n, struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST_sub* item_list)
{
	int cash, i; // [Stingor]
	nullpo_retr(-1,sd);
	...
	{
		ShowError("pc_paycash: Not enough points (vote=%d, kafra=%d) to cover the price (vote=%d, kafra=%d) (account_id=%d, char_id=%d).\n", sd->cashPoints, sd->kafraPoints, cash, points, sd->status.account_id, sd->status.char_id);
		return -1;
	}

	if( cash ){
		if( !sd->status.user_id || (Sql_Query( mmysql_handle, "UPDATE `vote` SET `vote` = `vote` - %d WHERE `user_id` = %d;", cash, sd->status.user_id ) != SQL_SUCCESS) ){
			Sql_ShowDebug( mmysql_handle ); // [Stingor]
			return -1;
		}
		sd->cashPoints -= cash;
		pc_setaccountreg(sd, add_str(CASHPOINT_VAR), sd->cashPoints);
		log_cash( sd, type, LOG_CASH_TYPE_CASH, -cash, 0); // [Stingor]
	}

	if( points ){
		pc_setaccountreg(sd, add_str(KAFRAPOINT_VAR), sd->kafraPoints - points);
		sd->kafraPoints -= points;
		log_cash( sd, type, LOG_CASH_TYPE_KAFRA, -points, 0); // [Stingor]
	}

	if (item_list != NULL) {
		for (i = 0; i < n; ++i)
			log_cash( sd, type, LOG_CASH_TYPE_CASH, item_list[i].amount, item_list[i].itemId);
	}
...
bool pc_dropitem(struct map_session_data *sd,int n,int amount)
{
	...
	if (!map_addflooritem(&sd->inventory.u.items_inventory[n], amount, sd->bl.m, sd->bl.x, sd->bl.y, 0, 0, 0, 2, 0, (pc_get_group_id(sd)>=80?1:0)))
		return false;
...
bool pc_isUseitem(struct map_session_data *sd,int n)
{
	...
	// [Stingor] -->
	if (nameid == 12103 && (!mapdata->flag[MF_NOBBRANCH] || mapdata_flag_gvg2(mapdata)) && pc_get_group_level(sd) < 80)
		return false;

	if (item->flag.dead_branch && (!mapdata->flag[MF_NOBRANCH] || mapdata_flag_gvg2(mapdata)) && pc_get_group_level(sd) < 50)
		return false;
	// [Stingor] <--
...
int pc_get_skillcooldown(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv) {
	...
	if (skill_id == PF_SPIDERWEB && sd->status.class_ == JOB_PROFESSOR)	// [Stingor]
		cooldown = 0;
...
static void pc_calcexp(struct map_session_data *sd, unsigned int *base_exp, unsigned int *job_exp, struct block_list *src)
{
	...
	if (*base_exp) {
		unsigned int exp = (unsigned int)(*base_exp + (double)*base_exp * (bonus + vip_bonus_base)/100.) * (event_exp?battle_config.mult_eventexp/100:1); // Stingor
...
#else
/// Pre-Renewal status point cost formula // [Stingor]
#define PC_STATUS_POINT_COST(low) (((low) <= 200) ? (1 + ((low) + 9) / 10) : (((low) <= 300) ? (1 + ( (2 *(low)) + 9) / 10) : (1 + ( (4 *(low)) + 9) / 10) ))
#endif
...
int64 pc_readparam(struct map_session_data* sd,int64 type)
{
	...
	switch(type) {
		...
		// [Stingor] -->
		case SP_NO_AMMO:         val = sd->special_state.no_ammo?1:0; break;
		case SP_NO_BOTTLE:       val = sd->special_state.no_bottle?1:0; break;
...
bool pc_setparam(struct map_session_data *sd,int64 type,int64 val_tmp)
{
	...
	case SP_MAXHP:
		...
		&& (sd->status.class_ == JOB_LORD_KNIGHT || sd->status.class_ == JOB_LORD_KNIGHT2) ) // [Stingor]
			sd->battle_status.max_hp = cap_value(val, 1, battle_config.max_hp_lkberserk);
		else
	...
	case SP_CASHPOINTS:
		...
		if (!sd->state.connect_new)
			log_cash(sd, LOG_TYPE_SCRIPT, LOG_CASH_TYPE_CASH, -(sd->cashPoints - cap_value(val, 0, MAX_ZENY)), 0); // [Stingor]
	...
	case SP_KAFRAPOINTS:
		if (val < 0)
			return false;
		if (!sd->state.connect_new)
			log_cash(sd, LOG_TYPE_SCRIPT, LOG_CASH_TYPE_KAFRA, -(sd->kafraPoints - cap_value(val, 0, MAX_ZENY)), 0); // [Stingor]
