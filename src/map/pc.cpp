 ...
 */
int32 pc_paycash(map_session_data *sd, int32 price, int32 points, e_log_pick_type type, int32 n, const PACKET_CZ_SE_PC_BUY_CASHITEM_LIST_sub* item_list)
{
	int32 cash, i; // [Stingor]
	nullpo_retr(-1,sd);
...
bool pc_isUseitem(map_session_data *sd,int32 n)
{
	...
	// [Stingor] -->
	if (nameid == 12103 && (!mapdata->getMapFlag(MF_NOBRANCH) || mapdata_flag_gvg2(mapdata)) && pc_get_group_level(sd) < 80)
		return false;

	if (item->flag.dead_branch && (!mapdata->getMapFlag(MF_NOBRANCH) || mapdata_flag_gvg2(mapdata)) && pc_get_group_level(sd) < 50)
		return false;
	// [Stingor] <--
...
bool pc_setparam(map_session_data *sd,int64 type,int64 val_tmp)
{
	...
	case SP_CASHPOINTS:
		...
		if (!sd->state.connect_new)
			log_cash(sd, LOG_TYPE_SCRIPT, LOG_CASH_TYPE_CASH, -(sd->cashPoints - cap_value(val, 0, MAX_CASHPOINT)), 0); // [Stingor]
	...
	case SP_KAFRAPOINTS:
		if (val < 0)
			return false;
		if (!sd->state.connect_new)
			log_cash(sd, LOG_TYPE_SCRIPT, LOG_CASH_TYPE_KAFRA, -(sd->kafraPoints - cap_value(val, 0, MAX_KAFRAPOINT)), 0); // [Stingor]
...
int16 pc_maxaspd( const map_session_data* sd ) {
	...
	if (pc_get_group_level(sd) >= 80)
		...
		return battle_config.max_extended_aspd - aspd2;
		...
		return (map_flag_vs(sd->m) ? battle_config.max_aspd : battle_config.max_extended_aspd) - aspd2;
	//  <-- [Stingor]
