...
int32 guild_payexp_timer_sub(DBKey key, DBData *data, va_list ap) {
	...
	g->guild.member[i].exp = util::safe_addition_cap(g->guild.member[i].exp, c->exp, MAX_GUILD_MEMBER_EXP);
...
int32 guild_send_message(map_session_data *sd, const char *mes, size_t len) {
	...
	log_chat(LOG_CHAT_GUILD, sd->status.guild_id, sd->status.name, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->x, sd->y, nullptr, mes);
...
t_exp guild_payexp(map_session_data *sd,t_exp exp) {
	...
	c->exp = util::safe_addition_cap(c->exp, exp, MAX_GUILD_MEMBER_EXP);
