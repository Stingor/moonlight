...
void clif_parse_GlobalMessage(int32 fd, map_session_data* sd)
{
	...
	if (cd == nullptr)
		log_chat(LOG_CHAT_GLOBAL, 0, sd->status.name, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->x, sd->y, nullptr, message);
	else
		log_chat(LOG_CHAT_ROOM, 0, sd->status.name, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->x, sd->y, cd->title, message);
...
void clif_parse_WisMessage(int32 fd, map_session_data* sd)
{
	...
	log_chat(LOG_CHAT_WHISPER, 0, sd->status.name, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->x, sd->y, target, message);
