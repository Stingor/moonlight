...
static inline unsigned char clif_bl_type(struct block_list *bl, bool walking) {
	...
	switch (type) {
	case BL_PC:    return (disguised(bl) && !pcdb_checkid(class_))? 0x1:0x0; //PC_TYPE
	case BL_ITEM:  return 0x2; //ITEM_TYPE
	case BL_SKILL: return 0x3; //SKILL_TYPE
	case BL_CHAT:  return 0x4; //UNKNOWN_TYPE
	case BL_MOB:   return pcdb_checkid(class_)?0x0:0x5; //NPC_MOB_TYPE
...
// Since walking NPCs are not supported on official servers, the client does not know how to handle it.
				   ...
				   return ( pcdb_checkid(class_) && walking ) ? 0x0 : 0x6; //NPC_EVT_TYPE
#else
...
int clif_send(const void* buf, int len, struct block_list* bl, enum send_target type)
{
	...
	case ALL_SAMEMAP: //All players on the same map
		...
		{
			if( bl->m == tsd->bl.m ){
				if (tsd && tsd->sc.option & OPTION_WINGS ) // [Stingor]
					clif_show_wings(tsd);
				WFIFOHEAD(tsd->fd, len);
				memcpy(WFIFOP(tsd->fd,0), buf, len);
				WFIFOSET(tsd->fd,len);
	...
	case BL_PC:
		{
			...
			sd->hom_teleport_tick = tick + battle_config.homunc_teleport_acd; // [Stingor]
			sd->aoe_teleport_tick = tick + battle_config.aoe_teleport_acd; // [Stingor]
			sd->refresh_tick = tick; // [Stingor]
	...
	case BL_PC:
		{
			TBL_PC *sd = ((TBL_PC*)bl);
//			clif_movepc(sd);
			if( disguised(bl) && map_getmapflag(sd->bl.m, MF_NODISGUISE) ) // [Stingor]
				pc_disguise(sd, 0);
			if(sd->state.size==SZ_BIG) // tiny/big players [Valaris]
	...
	for( i = 0; i < MAX_INVENTORY; i++ )
	{
		if( sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory_data[i] )
		{
			if( !pc_can_sell_item(sd, &sd->inventory.u.items_inventory[i], nd->subtype))
				continue;
			// [Stingor] -->
			if( !sd->state.sellstuff && (sd->inventory_data[i]->type == IT_CARD || sd->inventory.u.items_inventory[i].card[0] || 
																				   sd->inventory.u.items_inventory[i].card[1] || 
																				   sd->inventory.u.items_inventory[i].card[2] || 
																				   sd->inventory.u.items_inventory[i].card[3]) )
				continue; // No card or items with cards in shop sell list

			if( !sd->state.sellstuff && sd->inventory_data[i]->type == IT_AMMO ) { // No Arrow in shop sell list
				...
				if( sd->status.class_ == JOB_ASSASSIN_CROSS && sd->inventory_data[i]->nameid == 1771 ) continue;
			}
...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if(sd->state.connect_new) {
		...
		if (sd->sc.option&OPTION_FALCON)
			...
			clif_status_load( &sd->bl, EFST_RIDING, 1 );
#endif
		else if ( !map_flag_vs(sd->bl.m) && sd->sc.data[SC_ALL_RIDING]) // [Stingor]
	...
	if( sd->state.changemap ) {// restore information that gets lost on map-change
		...
		if (sd->status.show_equip == 1) // Stingor
		...
		if( !battle_config.pet_autofeed_always ){ //Stingor
...
void clif_parse_GlobalMessage(int fd, struct map_session_data* sd)
{
	...
	if( pc_get_group_level(sd) > 90 && (sd->sc.option & OPTION_INVISIBLE) ) { // [Stingor]
		return;
	}
	...
	if (cd == NULL)
		log_chat(LOG_CHAT_GLOBAL, 0, sd->status.name, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->bl.x, sd->bl.y, NULL, message);
	else
		log_chat(LOG_CHAT_ROOM, 0, sd->status.name, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->bl.x, sd->bl.y, cd->title, message);
...
void clif_parse_WisMessage(int fd, struct map_session_data* sd)
{
	...
	log_chat(LOG_CHAT_WHISPER, 0, sd->status.name, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->bl.x, sd->bl.y, target, message);
...
void clif_parse_HomAttack(int fd,struct map_session_data *sd)
{
	...
	if( DIFF_TICK(sd->hom_teleport_tick, gettick()) > 0 ) // [Stingor]
		return;
...
void clif_cashshop_open( struct map_session_data* sd, int tab ){
	...
	p.cashPoints = pc_readaccountreg(sd, add_str(CASHPOINT_VAR)); // [Stingor]
...
void clif_parse_PartyTick(int fd, struct map_session_data* sd)
{
	...
	clif_partytickack(sd, flag); // Stingor
}
...
static int clif_parse(int fd)
{
	...
	if (packet_len == -1) { // variable-length packet
		...
		if (packet_len < 4 || packet_len > 32768) {
			ShowWarning("clif_parse: Received packet 0x%04x specifies invalid packet_len (%d), disconnecting session #%d - '%s'.\n", cmd, packet_len, fd, ip2str(session[fd]->client_addr, NULL));
#ifdef DUMP_INVALID_PACKET
			ShowDump(RFIFOP(fd,0), RFIFOREST(fd));
#endif
			set_eof(fd);
