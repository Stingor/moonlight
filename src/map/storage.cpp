...
char storage_guild_storageopen(struct map_session_data* sd)
{
    ...
    // ShowWarning("storage_guild_storageopen debugmoon 2 gid:%d - storid:%d - amount:%d - maxamount:%d\n", gstor->id, gstor->stor_id, gstor->amount, gstor->max_amount);
    // ShowWarning("storage_guild_storageopen debugmoon 2 sd:%s - charid:%d\n", sd->status.name, sd->status.char_id);
...
enum e_guild_storage_log storage_guild_log_read_sub( struct map_session_data* sd, std::vector<struct guild_log_entry>& log, uint32 max ){
	...
	SqlStmt_BindColumn(stmt, 10, SQLDT_UINT64,   &tmp_unique_id,          0, NULL, NULL);
	for( j = 0; j < MAX_SLOTS; ++j )
...
bool storage_premiumStorage_load(struct map_session_data *sd, uint8 num, uint8 mode) {
	...
	ShowWarning("Premium storage (%d) introuvable\n", sd->premiumStorage.stor_id);
	// [Stingor] <--
