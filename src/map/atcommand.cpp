...
ACMD_FUNC(kill)
{
	...
	if (map_flag_vs(sd->bl.m) && pc_get_group_level(sd) < 80) { // [Stingor]
		clif_displaymessage(fd, "No @die in PvP");
		return -1;
 	}
...
ACMD_FUNC(monster)
{
	...
	// if (mob_id == MOBID_EMPERIUM) { // [Stingor]
...
ACMD_FUNC(reload) {
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		...
		||  prev_config.item_drop_equip_mvp_min   != battle_config.item_drop_equip_mvp_min
		||  prev_config.item_drop_equip_mvp_max   != battle_config.item_drop_equip_mvp_max
		||  prev_config.item_drop_equip_boss_min  != battle_config.item_drop_equip_boss_min
		||  prev_config.item_drop_equip_boss_max  != battle_config.item_drop_equip_boss_max
...
ACMD_FUNC(mapinfo) {
	...
	if (map_getmapflag(m_id, MF_NOBRANCH))
		strcat(atcmd_output, " NoDeadBranch |");
	if (map_getmapflag(m_id, MF_NOBBRANCH))
		strcat(atcmd_output, " NoBloodyBranch |");
	...
	if (map_getmapflag(m_id, MF_NODUEL)) // [Stingor]
		strcat(atcmd_output, " NoDuel |");
	if (map_getmapflag(m_id, MF_NOATTACKSKILL)) // [Stingor]
		strcat(atcmd_output, " NoAttackSkill |");
	if (map_getmapflag(m_id, MF_NOAOESKILL)) // [Stingor]
		strcat(atcmd_output, " NoAoeSkill |");
	if (map_getmapflag(m_id, MF_NOMVPCARD)) // [Stingor]
		strcat(atcmd_output, " NoMvpCard |");
	if (map_getmapflag(m_id, MF_NOMAIL)) // [Stingor]
 ...
 *------------------------------------------*/
#define MAX_SKILLID_PARTIAL_RESULTS 10
...
ACMD_FUNC(npctalk)
{
	...
	snprintf(temp, sizeof(temp), "%s : %s", nd->name, mes);	// [Stingor] unique_name
		...
		if (mob->mexp) {
			...
			for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				if (mvppercent > 0) {
					...
					std::string itemlink = create_item_link_simple(item_data->nameid);
					if (j == 1)
						sprintf(atcmd_output2, " %s %02.02f%%", itemlink.c_str(), mvppercent);
					else
						sprintf(atcmd_output2, " - %s %02.02f%%", itemlink.c_str(), mvppercent);
					strcat(atcmd_output, atcmd_output2);
				}
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->mob[0].chance == 0) {
			...
			for (j=0; j < MAX_SEARCH && item_data->mob[j].chance > 0; j++)
			{
				...
				int mobid = item_data->mob[j].id;
				int drop_rate_bonus = 0;

				// Add class and race specific bonuses
				drop_rate_bonus += sd->dropaddclass[mob_db(mobid)->status.class_] + sd->dropaddclass[CLASS_ALL];
				drop_rate_bonus += sd->dropaddrace[mob_db(mobid)->status.race] + sd->dropaddrace[RC_ALL];
...
#ifdef RENEWAL_DROP
				if( battle_config.atcommand_mobinfo_type )
					dropchance = dropchance * pc_level_penalty_mod(mob_db(mobid)->lv - sd->status.base_level, mob_db(mobid)->status.class_, mob_db(mobid)->status.mode, 2) / 100;
#endif
				...
				if (spawns.size() < 1)
					sprintf(atcmd_output, "- %s (%d): %02.02f%% - %s", mob_db(mobid)->jname, mobid, (dropchance/100.), msg_txt(sd,1290) ); // Pas de spawn - Instance/Quest/Event
				else if( spawns.size() == 1 ) {
					int16 mapid = map_mapindex2mapid(spawns[0].mapindex);
					if( mapid > 1 ) {
						if( spawns[0].qty > 1 )
							sprintf(atcmd_output, "- %s (%d): %02.02f%% - %d mobs dans la map %s", mob_db(mobid)->jname, mobid, (dropchance/100.), spawns[0].qty, map_getmapdata(mapid)->name);
						else
							sprintf(atcmd_output, "- %s (%d): %02.02f%% - 1 mob dans la map %s", mob_db(mobid)->jname, mobid, (dropchance/100.), map_getmapdata(mapid)->name);
					}
				}
...
ACMD_FUNC(vote)
{
	...
	if( !strcmpi(parent_cmd,"vote") )
	{
		if( value > 0 ) {
			...
			if (-value > sd->cashPoints) //By command, if cash < value, force it to remove all
				value = -sd->cashPoints;
			if( (ret=pc_paycash(sd, -value, 0, LOG_TYPE_COMMAND, 0, NULL)) >= 0){
				// If this option is set, the message is already sent by pc function
				if( !battle_config.cashshop_show_points ){
	...
	{ // @points
		if( value > 0 ) {
			...
			if( (ret=pc_paycash(sd, 0, -value, LOG_TYPE_COMMAND, 0, NULL)) >= 0){
