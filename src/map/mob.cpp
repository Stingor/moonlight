	...
	if(mvp_sd && md->db->mexp > 0 && !md->special_state.ai) {
		...
		if( !(map_getmapflag(m, MF_NOMVPLOOT) || type&1) ) {
			...
			for(i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				if( event_drop ) { // Stingor
					temp = apply_rate(temp, battle_config.mult_eventdrop);
...
int mobskill_use(struct mob_data *md, t_tick tick, int event)
{
	...
	for (n = 0; n < md->db->maxskill; i++, n++) {
		...
		int casttime = ms[i].casttime;

		if (md->status.dex >= 50)
			casttime = ms[i].casttime - md->status.dex;
		...
		{	//Ground skill.
			...
			if (!battle_check_range(&md->bl, bl, skill_get_range2(&md->bl, ms[i].skill_id, ms[i].skill_lv, true)) ||
				!unit_skilluse_pos2(&md->bl, x, y, ms[i].skill_id, ms[i].skill_lv, casttime, ms[i].cancel))
			{
				map_freeblock_unlock();
				continue;
			}
			...
			map_freeblock_lock();
			if (!battle_check_range(&md->bl, bl, skill_get_range2(&md->bl, ms[i].skill_id, ms[i].skill_lv, true)) ||
				!unit_skilluse_id2(&md->bl, bl->id, ms[i].skill_id, ms[i].skill_lv, casttime, ms[i].cancel))
			{
				map_freeblock_unlock();
				continue;
			}
