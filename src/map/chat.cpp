...
int chat_joinchat(map_session_data* sd, int chatid, const char* pass)
{
	...
	if( sd->bl.m == map_mapname2mapid(MAP_GONRYUN) && sd->bl.y <= 131 && pc_get_group_id(sd) < 60 ) { // [Stingor]
		clif_displaymessage(sd->fd, msg_txt(sd,1834));
		return 0;
	}
...
int chat_changetitlenpcchat(struct npc_data* nd, const char* title) // [Stingor]
{
	struct chat_data* cd;
	nullpo_ret(nd);
