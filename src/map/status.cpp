...
// ----- HP MAX CALCULATION -----
	...
	if (sd->status.base_level < 100)
		...
		base_status->max_hp = cap_value(base_status->max_hp,1,(uint32)battle_config.max_hp_lkberserk);
	else
	...
	if(flag[SCB_MAXHP]) {
		if( bl.type == BL_PC ) {
			...
			if (sd->status.base_level < 100)
				...
				status->max_hp = umin(status->max_hp,(uint32)battle_config.max_hp_lkberserk);
			else
