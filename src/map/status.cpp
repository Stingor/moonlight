...
int status_damage(struct block_list *src,struct block_list *target,int64 dhp, int64 dsp, t_tick walkdelay, int flag, uint16 skill_id)
{
	...
	**/
	...
	if( sc && sc->data[SC_KAIZEL] && !map_flag_vs(target->m) ) { // flag&8 = disable Kaizel
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	// Hit
	stat = status->hit;
	stat += cap_value(level, 1, battle_config.maxstatlevelcalc) + status->dex; // [Stingor]
	status->hit = cap_value(stat, 1, SHRT_MAX);
	// Flee
	stat = status->flee;
	stat += cap_value(level, 1, battle_config.maxstatlevelcalc) + status->agi; // [Stingor]
	status->flee = cap_value(stat, 1, SHRT_MAX);
	// Def2
	stat = status->def2;
	stat += status->vit;
...
int status_isimmune(struct block_list *bl)
{
	...
	if(bl->type == BL_PC &&
		...
		if( map_flag_vs(bl->m) && battle_config.pvp_gtb_nerf ) // Nerf gtb en pvp [Stingor]
			((TBL_PC*)bl)->special_state.no_magic_damage = battle_config.pvp_gtb_nerf;
...
t_tick status_get_sc_def(struct block_list *src, struct block_list *bl, enum sc_type type, int rate, t_tick tick, unsigned char flag)
{
	...
	if (type != SC_SMA && type != SC_KAIZEL && type != SC_KAUPE && status_isimmune(bl) ) {
	...
	switch (type) {
		...
		case SC_STONE:
			...
			break;
		case SC_FREEZE:
			sc_def = status->mdef*(battle_config.mdefimmunefreeze); // [Stingor]
			sc_def2 = status->luk*(battle_config.lukimmunefreeze) + status_get_lv(bl)*10 - status_get_lv(src)*10; // [Stingor]
		...
		case SC_CURSE:
			...
			sc_def2 = status->luk*10 - min(170, status_get_lv(src)*10); // Curse only has a level penalty and no resistance // [Stingor]
		...
		case SC_WHITEIMPRISON:
			...
			break;
		case SC_BURNING:
			if( sd && sd->status.agi >= battle_config.burningnerf ) // [Stingor]
				sc_def2 = 50000;
			tick_def2 = 150*status->luk + 250*status->agi;
			break;
		case SC_FREEZING:
			tick_def2 = (status->vit + status->dex)*50;
			break;
		...
		case SC_ELECTRICSHOCKER:
			...
			break;
		case SC_CRYSTALIZE:
			if( sd && sd->status.vit >= battle_config.crystalizenerf ) // [Stingor]
				sc_def2 = 50000;
			tick_def2 = (sd ? (sd->status.vit * 2) : status_get_base_status(bl)->vit) * 100; // [Stingor]
			break;
	...
	// Minimum durations
	switch (type) {
		case SC_BURNING: // Stingor
		case SC_CRYSTALIZE:
			tick = i64max(tick, 3000); // Minimum duration 3s
			break;
		case SC_ANKLE:
		case SC_MARSHOFABYSS:
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	case SC_STONE:
		...
		break;
	case SC_COMA:	// [Stingor] Dark and undead immune coma
		if( bl->type == BL_PC && (status->def_ele == ELE_DARK || status->def_ele == ELE_UNDEAD) )
			return 0;
	case SC_FREEZE:
...
#endif
		if( battle_config.disable_edp_berserk )
			status_change_end(bl, SC_EDP, INVALID_TIMER);
		break;
	case SC_ASSUMPTIO:
#ifndef RENEWAL
...
// 			break;
	...
	if( sd && sd->status.party_id && ( // SPB [Stingor]
