...
void SkillDispell::castendNoDamageId(block_list *src, block_list *target, uint16 skill_lv, t_tick tick, int32& flag) const {
	...
	if (flag&1 || (i = skill_get_splash(getSkillId(), skill_lv)) < 1) {
		...
		// [Stingor] -->
		if (tsc && tsc->getSCE(SC_PRESERVE) && status_isimmune(target) && battle_config.gtb_nodispell_preserve && rnd() % 100 <= battle_config.gtb_nodispell_preserve)
			return;

		if (status_isimmune(target) && map_flag_vs(target->m) && !battle_config.gtb_dispell_nerf)
			return;

		if (status_isimmune(target) && !map_flag_vs(target->m))
			return;
		// [Stingor] <--
