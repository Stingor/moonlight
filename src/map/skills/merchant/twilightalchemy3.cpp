...
void SkillTwilightAlchemy3::castendNoDamageId(block_list* src, block_list* target, uint16 skill_lv, t_tick tick, int32& flag) const {
	...
	if (sd) { // [Stingor]
		...
		if (   !(acid_idx = skill_can_produce_mix(sd, ITEMID_ACID_BOTTLE, -1, 50))
			|| !(fire_idx = skill_can_produce_mix(sd, ITEMID_FIRE_BOTTLE, -1, 50))
			||  ebottle < 100
		) {
			clif_skill_fail( *sd, getSkillId() );
			return;
		...
		clif_skill_nodamage(src, *target, getSkillId(), skill_lv);
		skill_produce_mix(sd, getSkillId(), ITEMID_ACID_BOTTLE, 0, 0, 0, amount, acid_idx-1);
		skill_produce_mix(sd, getSkillId(), ITEMID_FIRE_BOTTLE, 0, 0, 0, amount, fire_idx-1);
	}
}
