...
static int32 battle_skill_damage_skill(struct block_list *src, struct block_list *target, uint16 skill_id) {
	...
	{
		if( skill_id == AC_DOUBLE && ((TBL_PC*)src)->status.class_ == JOB_STALKER && battle_config.nerfdstrafestalk )
			return (damage->rate[battle_skill_damage_type(target)] / 2); // [Stingor]
...
static bool is_attack_critical(struct Damage* wd, struct block_list *src, struct block_list *target, int32 skill_id, int32 skill_lv, bool first_call)
{
	...
	if( sstatus->cri )
	{
		...
		if (sd) {
			...
			if ((!skill_id || battle_config.unlockcrit) && is_skill_using_arrow(src, skill_id)) {
				cri += sd->bonus.arrow_cri;
				if (battle_config.unlockcrit) // [Stingor]
...
//			wd->damage = sstatus->max_hp * 9/100;
			if( sd && sd->status.class_ == JOB_PALADIN ) // [Stingor]
				wd->damage = cap_value(sstatus->max_hp,100,battle_config.paladin_sacri_max_hp) * 9/100;
			else
				wd->damage = cap_value(sstatus->max_hp,100,battle_config.all_sacri_max_hp) * 9/100;
			wd->damage2 = 0;
#ifdef RENEWAL
...
#else
		case NJ_ISSEN: // [Stingor]
			wd->damage = 40 * (sstatus->str >= battle_config.nj_issenstr ? battle_config.nj_issenstr : sstatus->str) + sstatus->hp * 8 * skill_lv / 100;
			wd->damage2 = 0;
			break;
		case LK_SPIRALPIERCE:
		case ML_SPIRALPIERCE:
...
static int32 battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case MO_INVESTIGATE:
			...
			break;
		case MO_EXTREMITYFIST:
			skillratio += 100 * (7 + (sstatus->sp >= battle_config.azuranerfsp ? battle_config.azuranerfsp : sstatus->sp) / 10); // [Stingor]
#ifdef RENEWAL
			if (wd->miscflag&1)
		...
		case WS_CARTTERMINATION:
			...
			if (sd && sd->cart_weight)
				if( battle_config.newcarttermform )
		...
		case TK_JUMPKICK:
			...
			if (sc && sc->getSCE(SC_COMBO) && sc->getSCE(SC_COMBO)->val1 == skill_id)
			{
				...
				if (target && target->type == BL_PC) // [Stingor]
					skillratio += -100 + battle_config.jump_kick_pcnerf * status_get_lv(src); //Tumble formula [4%*baselevel]
				else
					skillratio += -100 + 4 * status_get_lv(src); //Tumble formula [4%*baselevel]
			}
			else if (wd->miscflag) {
...
static void battle_attack_sc_bonus(struct Damage* wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sd != nullptr && anger_id < MAX_PC_FEELHATE && (anger_level = pc_checkskill(sd, sg_info[anger_id].anger_id))) {
		int32 skillratio;
		if (target && target->type == BL_PC) // [Stingor]
			skillratio = (int32)(sd->status.base_level / battle_config.hatred_pcnerf ) + sstatus->dex + sstatus->luk;
		else
...
static void battle_calc_defense_reduction(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (nk[NK_SIMPLEDEFENSE]) {
		...
		ATK_RATE2(wd->damage, wd->damage2,
			attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_R) ?100:(is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R) ? (int64)is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R)*(def1+(vit_def / battle_config.piercevitdef)) : (100-def1)),
			attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_L) ?100:(is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L) ? (int64)is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L)*(def1+(vit_def / battle_config.piercevitdef)) : (100-def1))
		);
...
static void battle_calc_attack_plant(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if( attack_hits && (status_get_class(target) == MOBID_EMPERIUM || status_get_class(target) == MOBID_GUARDIAN_STONE1 || status_get_class(target) == MOBID_GUARDIAN_STONE2) ) { // [Stingor]
			...
			default: {
				...
				switch(skill_id) {
					...
					case SL_SMA: //Base damage is 40% + lv%
						...
						if (target && target->type == BL_PC) // [Stingor]
							skillratio += -60 + (status_get_lv(src) / battle_config.esma_pcnerf);
						else
							skillratio += -60 + status_get_lv(src);
						break;
					case NJ_KOUENKA:
...
#ifndef RENEWAL
		case BA_DISSONANCE:
			md.damage = 30 + skill_lv * 10 * (sstatus->dex); // [Stingor]
			if (sd)
				md.damage += 3 * pc_checkskill(sd,BA_MUSICALLESSON); // [Stingor]
		...
		case NJ_ZENYNAGE:
			if (sd) {
				if (md.damage > sd->status.zeny)
					md.damage = sd->status.zeny * 2; // [Stingor]
				pc_payzeny( sd, static_cast<int32>( cap_value(md.damage, INT_MIN, INT_MAX) / 2 ), LOG_TYPE_CONSUME );	// [Stingor]
			}
			break;
	}
