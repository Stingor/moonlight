...
static struct wing_group wingsgroup_db[2];
...
t_itemid wingsdb_search(t_itemid id)
{
	...
	if ( j == ARRAYLENGTH(wingsgroup_db) )
		return id;
	return wingsgroup_db[j].view_id2;
}
...
/** Checks if item is card type or not
* @param id Item data
* @return True if item is card, false otherwise
*/
bool itemdb_iscard2(struct item_data *id) {
	nullpo_ret(id);
	if (id->type == IT_CARD)
		return true;
