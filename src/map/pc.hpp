...
public:
	...
	struct s_state {
		...
		bool pending_storeall; // [Stingor] @storeall N is waiting for the requested premium storage to actually finish loading
		bool pending_storeall_cards_only; // [Stingor] ...et ce rangement differe ne concerne QUE les cartes (@storecard). Toujours accompagne de pending_storeall, qui reste LE drapeau « un rangement est arme » : c'est lui que testent les gardes des deux commandes.
	...
	/* Stingor */
	t_tick hom_teleport_tick;
	t_tick aoe_teleport_tick;
	t_tick refresh_tick;
...
#define EQP_COSTUME_HEAD EQP_COSTUME_HELM
...
#define pc_istrading2(sd)     ( (sd)->npc_id || (sd)->state.vending || (sd)->state.autotrade || (sd)->state.buyingstore || (sd)->state.trading || sd->state.banking || sd->state.callshop )
...
#define pc_iswingiswingon(sd)       ( (sd)->sc.option&OPTION_WINGS )
...
// Special Shop System
int32 pc_paycash( map_session_data *sd, int32 price, int32 points, e_log_pick_type type, int32 n = 0, const PACKET_CZ_SE_PC_BUY_CASHITEM_LIST_sub* item_list = nullptr);
