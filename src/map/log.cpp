...
static char log_chattype2char(e_log_chat_type type)
{
	switch( type )
	{
		...
		case LOG_CHAT_ROOM:     return 'R';  // (R)oom
	}
...
void log_pick(int id, int16 m, e_log_pick_type type, int amount, struct item* itm)
{
	...
	if( log_config.sql_logs )
	{
		...
		StringBuf_Printf(&buf, ") VALUES(NOW(),'%u','%c','%u','%d','%d','%s','%" PRIu32 "','%d'",
...
/// logs chat
void log_chat(e_log_chat_type type, int type_id, const char* src_charname, int src_accid, const char* mapname, int x, int y, const char* dst_charname, const char* message)
{
	...
	if( log_config.sql_logs ) {
		...
		stmt = SqlStmt_Malloc(logmysql_handle);
		if( SQL_SUCCESS != SqlStmt_Prepare(stmt, LOG_QUERY " INTO `%s` (`time`, `type`, `type_id`, `src_charid`, `src_accountid`, `src_map`, `src_map_x`, `src_map_y`, `dst_charname`, `message`) VALUES (NOW(), '%c', '%d', '%s', '%d', '%s', '%d', '%d', ?, ?)", log_config.log_chat, log_chattype2char(type), type_id, src_charname, src_accid, mapname, x, y)
	...
	else
	{
		...
		time(&curtime);
		strftime(timestring, sizeof(timestring), log_timestamp_format, localtime(&curtime));
		fprintf(logfp, "%s - %c,%d,%s,%d,%s,%d,%d,%s,%s\n", timestring, log_chattype2char(type), type_id, src_charname, src_accid, mapname, x, y, dst_charname, message);
		fclose(logfp);
	}
}

/// logs cash transactions
void log_cash( struct map_session_data* sd, e_log_pick_type type, e_log_cash_type cash_type, int amount , int itemid){
	nullpo_retv( sd );
	...
	if( log_config.sql_logs ){
		if( SQL_ERROR == Sql_Query( logmysql_handle, LOG_QUERY " INTO `%s` ( `time`, `char_id`, `type`, `cash_type`, `itemid`, `amount`, `map` ) VALUES ( NOW(), '%d', '%c', '%c', '%d', '%d', '%s' )",
			log_config.log_cash, sd->status.char_id, log_picktype2char( type ), log_cashtype2char( cash_type ), itemid, amount, mapindex_id2name( sd->mapindex ) ) )
		{
			Sql_ShowDebug( logmysql_handle );
			return;
		}
